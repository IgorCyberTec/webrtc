...
def capture_camera_frames():
    """
    Captura frames da câmera do robô e os coloca na fila de frames.
    """
    ...
    for index in range(5):
        ...
        if cap.isOpened():
            logging.info(f"Câmera aberta no índice {index}.")
            break
        else:
            logging.warning(f"Tentando abrir a câmera no índice {index} falhou.")
            cap.release()
            cap = None

    if not cap or not cap.isOpened():
        logging.error("Nenhuma câmera disponível. Usando frames pretos.")
        while True:
            # Criar uma imagem preta
            frame = np.zeros((120, 160, 3), dtype=np.uint8)
            frame_queue.put(frame)
            time.sleep(0.05)  # Aproximadamente 20 FPS
    else:
        while True:
            ret, frame = cap.read()
            if not ret:
                logging.warning("Frame não recebido da câmera.")
                # Colocar uma imagem preta na fila
                frame = np.zeros((120, 160, 3), dtype=np.uint8)
            else:
...
def process_lidar_data():
    ...
    while True:
        try:
            ...
            # Por exemplo, decodificar dados voxel_map_compressed
            # Isso dependerá do formato específico dos dados recebidos
            # Aqui, apenas imprimimos os dados para depuração
            logging.info(f"Dados do LIDAR recebidos: {data}")
            # Implementar processamento real conforme necessário
        except Empty:
            continue

def lidar_callback(message):
    """
    Callback para lidar com mensagens recebidas do LIDAR.

    Parâmetro:
    - message: Mensagem recebida contendo dados do LIDAR.
    """
    # Colocar os dados recebidos na fila do LIDAR
    lidar_queue.put(message["data"])

# =========================================
# Loop Asyncio para Conexão WebRTC
# =========================================

init_motion_mode = 'normal'  # Modo de movimento inicial

def run_asyncio_loop():
    """
    Executa o loop asyncio para gerenciar a conexão WebRTC e assinatura dos dados do LIDAR.
    """
    async def setup():
        try:
            await conn.connect()
            ...
            if response['data']['header']['status']['code'] == 0:
                ...
                logging.info(f"Modo atual: {current_motion_switcher_mode}")
                if current_motion_switcher_mode != init_motion_mode:
                    logging.info(f"Alterando modo de movimento para '{init_motion_mode}'...")
                    await conn.datachannel.pub_sub.publish_request_new(
                        RTC_TOPIC["MOTION_SWITCHER"],
                        {"api_id": 1002, "parameter": {"name": init_motion_mode}}
                    )
                    await asyncio.sleep(5)
            
            # Publicar mensagem para ligar o LIDAR
            await conn.datachannel.pub_sub.publish_request_new(
                RTC_TOPIC["rt/utlidar/switch"],
                {"data": "on"}
            )
            logging.info("LIDAR ligado.")

            # Subscribir-se aos dados do LIDAR
            conn.datachannel.pub_sub.subscribe("rt/utlidar/voxel_map_compressed", lidar_callback)
            logging.info("Subscreveu-se aos dados do LIDAR.")
...
class RobotEnv(gym.Env):
    ...
    def preprocess_lidar(self, lidar_data):
        ...
        try:
            ...
            if isinstance(lidar_data, str):
                ...
                distances = distances[:self.lidar_size]  # Limitar a 360
                if len(distances) < self.lidar_size:
                    distances += [0.0] * (self.lidar_size - len(distances))
                ...
                distances = lidar_json.get("distances", [])
                distances = distances[:self.lidar_size]
                if len(distances) < self.lidar_size:
                    distances += [0.0] * (self.lidar_size - len(distances))
                processed_lidar = np.array(distances, dtype=np.float32)
            else:
                # Outro formato, ajuste conforme necessário
                processed_lidar = np.zeros(self.lidar_size, dtype=np.float32)
        except Exception as e:
        ...
        if self.lidar_size % 120 == 0:
            channels = self.lidar_size // 120  # 3
    ...
    def get_camera_frame(self):
        ...
        try:
            ...
            processed_frame = self.preprocess_observation(frame)
    ...
    def get_lidar_frame(self):
        ...
        try:
            ...
            processed_lidar = self.preprocess_lidar(lidar_data)
            return processed_lidar
        except Empty:
    ...
    def step(self, action):
        ...
        # Verificar se uma ação está em execução
        if self.action_remaining_steps > 0:
            ...
            # Continuar executando a ação atual
            # Aqui, você pode ajustar os comandos de movimento conforme necessário
            ...
            # Reshape do LIDAR para (120, 3) e expandir para adicionar como canais
            lidar_observation_expanded = lidar_observation.reshape(120, 3)
            
            # Expandir LIDAR para ter a mesma largura da câmera
            # Primeiro, adicionar uma dimensão para a largura
            lidar_observation_expanded = lidar_observation_expanded[:, np.newaxis, :]  # (120, 1, 3)
            
            # Repetir os dados do LIDAR ao longo da largura para obter (120,160,3)
            lidar_observation_expanded = np.repeat(lidar_observation_expanded, self.camera_shape[1], axis=1)  # (120,160,3)
...
def display_frames():
    ...
    try:
        ...
        frame = np.rot90(frame)  # Rotacionar a imagem se necessário
